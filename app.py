...
def deafult_call(call,cuisine_inputs,highlight_inputs,establishment,locality,delivery,start,end,rating_cost):
    cuisines_list = []
    with open('B:/My_projects/restaurant_analysis/static/text/cuisines.txt') as f:
        for line in f.readlines():
            cuisines_list.append(line.strip(',\n'))

    highlights_list = []
    with open('B:/My_projects/restaurant_analysis/static/text/highlights.txt') as f:
        for line in f.readlines():
            highlights_list.append(line.strip(',\n'))

    establishment_list = []
    with open('B:/My_projects/restaurant_analysis/static/text/establishment.txt') as f:
        for line in f.readlines():
            establishment_list.append(line.strip(',\n'))

    locality_list = []
    with open('B:/My_projects/restaurant_analysis/static/text/locality.txt') as f:
        for line in f.readlines():
    ...
    with keras.backend.get_session().graph.as_default():
        if (call==2):
            predict_cost = load_model('B:/My_projects/restaurant_analysis/static/text/predict_cost.h5')
            scaler_cost = joblib.load('B:/My_projects/restaurant_analysis/static/text/scaler_cost.save')
    ...
    # To predict Rating:
        if call==1:
            predict_rating = load_model('B:/My_projects/restaurant_analysis/static/text/predict_rating.h5')
            scaler_rating = joblib.load('B:/My_projects/restaurant_analysis/static/text/scaler_rating.save')
